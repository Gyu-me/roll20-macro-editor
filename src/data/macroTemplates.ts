import type { MacroTemplate } from "@/types/macro";

const JUDGEMENT_STYLE = `text-decoration: none; font-style: normal !important; text-align: center; display: inline-block; color: white; letter-spacing: -1px; border-radius: 20px; padding: 5px 15px;margin-left: -4px;background-image: linear-gradient(135deg, #그라데이션시작, #그라데이션끝 &#41;`;

export const macroTemplates: MacroTemplate[] = [
  // ────────────────────────────────────────
  // basic
  // ────────────────────────────────────────
  {
    id: "narration",
    categoryId: "basic",
    order: 1,
    name: "나레이션",
    memo: "기본 묘사 출력",
    previewText: "내용을 입력하세요",
    keywords: ["나레이션", "desc", "묘사", "기본"],
    code: `/desc ?{내용}`,
  },
  {
    id: "narration-normal",
    categoryId: "basic",
    order: 2,
    name: "나레이션 (기울임 제거)",
    memo: "Roll20 기본 기울임 없이 출력",
    previewText: "할말",
    keywords: ["나레이션", "기울임 제거", "normal", "desc"],
    code: `/desc [?{할말}](#" style="text-decoration: none; font-style:normal; color:#333333;)`,
  },
  {
    id: "narration-space",
    categoryId: "basic",
    order: 3,
    name: "나레이션 공백",
    memo: "로그 사이에 빈 줄 추가",
    previewText: "빈 줄",
    keywords: ["공백", "빈줄", "나레이션", "간격"],
    code: `/desc [ ](#" style="text-decoration: none; display:block;)`,
  },
  {
    id: "npc-speech",
    categoryId: "basic",
    order: 4,
    name: "NPC 대사",
    memo: "NPC 이름으로 대사 출력",
    previewText: "뭐라고",
    keywords: ["NPC", "대사", "as", "말"],
    code: `/as "?{누구}" ?{뭐라고}`,
  },
  {
    id: "emote-orange",
    categoryId: "basic",
    order: 5,
    name: "주황색 / emote",
    memo: "캐릭터 행동문 출력 (주황색)",
    previewText: "캐릭터가 행동했다",
    keywords: ["emote", "주황색", "행동", "emas"],
    code: `/emas " " ?{내용}`,
  },
  {
    id: "markdown-style",
    categoryId: "basic",
    order: 6,
    name: "기본 텍스트 스타일",
    memo: "기울임·굵게·둘 다",
    previewText: "기울임 / 굵게 / 둘 다",
    keywords: ["기울임", "굵게", "볼드", "마크다운"],
    code: `*기울임*\n\n**굵게**\n\n***둘 다***`,
  },

  // ────────────────────────────────────────
  // roll
  // ────────────────────────────────────────
  {
    id: "roll-basic",
    categoryId: "roll",
    order: 1,
    name: "기본 주사위",
    memo: "공개 주사위 굴림",
    previewText: "1d100",
    keywords: ["주사위", "roll", "공개", "다이스"],
    code: `/r 1d?{면수|100}`,
  },
  {
    id: "roll-inline",
    categoryId: "roll",
    order: 2,
    name: "인라인 주사위",
    memo: "문장 안에 넣는 주사위",
    previewText: "[[1d100]]",
    keywords: ["인라인", "주사위", "inline", "다이스"],
    code: `[[1d?{면수|100}]]`,
  },
  {
    id: "judgement",
    categoryId: "roll",
    order: 3,
    name: "판정 버튼",
    memo: "기능명 판정 표시 — 컬러 프리셋 10종 + 커스텀",
    previewText: "✷ 기능 판정 ✷",
    keywords: ["판정", "버튼", "기능", "그라데이션", "컬러"],
    code: `/desc [　✷　?{기능|기능명} 판정　✷　](#" style="${JUDGEMENT_STYLE})`,
    variants: [
      {
        id: "basic",
        name: "판정",
        code: `/desc [　✷　?{기능|기능명} 판정　✷　](#" style="${JUDGEMENT_STYLE})`,
      },
      {
        id: "modifier",
        name: "증감수치",
        code: `/desc [　✷　?{기능|기능명} ?{증감수치|+0}　✷　](#" style="${JUDGEMENT_STYLE})`,
      },
    ],
    colorPresets: [
      { name: "분홍", values: { 그라데이션시작: "#FF0053", 그라데이션끝: "#FFD444" } },
      { name: "연두", values: { 그라데이션시작: "#056656", 그라데이션끝: "#87BD00" } },
      { name: "파랑", values: { 그라데이션시작: "#012C8F", 그라데이션끝: "#2191BB" } },
      { name: "보라", values: { 그라데이션시작: "#5300CC", 그라데이션끝: "#9536FF" } },
      { name: "봄", values: { 그라데이션시작: "#FF6F7D", 그라데이션끝: "#A7D3FF" } },
      { name: "여름", values: { 그라데이션시작: "#60B8FF", 그라데이션끝: "#C5E568" } },
      { name: "가을", values: { 그라데이션시작: "#AA0000", 그라데이션끝: "#FFA430" } },
      { name: "겨울", values: { 그라데이션시작: "#153869", 그라데이션끝: "#B4D0EB" } },
      { name: "새벽", values: { 그라데이션시작: "#3966FF", 그라데이션끝: "#FFBD88" } },
      { name: "밤", values: { 그라데이션시작: "#000911", 그라데이션끝: "#00385D" } },
    ],
    fieldDefaults: {
      "color_그라데이션시작": "#FF0053",
      "color_그라데이션끝": "#FFD444",
    },
  },

  // ────────────────────────────────────────
  // secret
  // ────────────────────────────────────────
  {
    id: "whisper-gm",
    categoryId: "secret",
    order: 1,
    name: "GM 귓말",
    memo: "GM에게만 내용 전달",
    previewText: "할말",
    keywords: ["귓말", "gm", "비밀", "whisper"],
    code: `/w gm ?{할말}`,
  },
  {
    id: "whisper-character",
    categoryId: "secret",
    order: 2,
    name: "캐릭터 귓말",
    memo: "특정 캐릭터에게 귓말",
    previewText: "내용",
    keywords: ["귓말", "캐릭터", "비밀", "whisper"],
    code: `/w ?{보낼 캐릭터} ?{내용}`,
  },
  {
    id: "secret-roll-basic",
    categoryId: "secret",
    order: 3,
    name: "비밀 주사위 (기본)",
    memo: "GM에게만 CoC 판정 결과 전달",
    previewText: "GM 비밀 판정",
    keywords: ["비밀", "주사위", "coc", "gm"],
    code: `/w gm &{template:coc-1}{{name=?{기능}}}{{success=[[?{기능치}]]}}{{hard=[[floor(?{기능치}/2)]]}}{{extreme=[[floor(?{기능치}/5)]]}}{{roll1=[[1d100]]}}`,
  },
  {
    id: "secret-roll-bonus",
    categoryId: "secret",
    order: 4,
    name: "비밀 주사위 (보너스)",
    memo: "보너스 주사위 포함 CoC 판정",
    previewText: "GM 보너스 판정",
    keywords: ["비밀", "주사위", "coc", "보너스", "gm"],
    code: `/w gm &{template:coc}{{name=?{기능}}}{{success=[[?{기능치}]]}}{{hard=[[floor(?{기능치}/2)]]}}{{extreme=[[floor(?{기능치}/5)]]}}{{roll1=[[1d100]]}}{{roll2=[[1d100]]}} {{roll3=[[1d100]]}}`,
  },
  {
    id: "blur-1",
    categoryId: "secret",
    order: 5,
    name: "블러 처리 1",
    memo: "검은 그림자로 내용을 가림",
    previewText: "블러 내용",
    keywords: ["블러", "스포일러", "비밀", "가리기"],
    code: `[?{블러 내용|}](#" style="text-decoration: none; color:rgba(0,0,0,0&rpar;; text-shadow:5px 0px 10px black,-5px 0px 10px black; font-size:11px;)`,
  },
  {
    id: "blur-2",
    categoryId: "secret",
    order: 6,
    name: "블러 처리 2",
    memo: "이미지 패턴으로 가리기 (진함)",
    previewText: "블러 내용",
    keywords: ["블러", "스포일러", "비밀", "가리기"],
    code: `[?{블러 내용|}](#" style="color:#000000; font-size: 14px; background-image:url('https://imgur.com/H79zWMS.png'&#41;; background-size: height cover; background-position: center center; background-size: height 300px; background-repeat:repeat-x scroll; text-decoration:none;)`,
  },
  {
    id: "blur-3",
    categoryId: "secret",
    order: 7,
    name: "블러 처리 3",
    memo: "이미지 패턴으로 가리기 (연함)",
    previewText: "블러 내용",
    keywords: ["블러", "스포일러", "비밀", "가리기"],
    code: `[?{블러 내용|}](#" style="color:#000000; font-size: 14px; background-image:url('https://imgur.com/Y5lJTpb.png'&#41;; background-size: height cover; background-position: center center; background-size: height 300px; background-repeat:repeat-x scroll; text-decoration:none;)`,
  },
  {
    id: "blur-4",
    categoryId: "secret",
    order: 8,
    name: "블러 처리 4",
    memo: "이미지 패턴 가리기 (200px)",
    previewText: "블러 내용",
    keywords: ["블러", "스포일러", "비밀", "가리기"],
    code: `[?{블러 내용|}](#" style="color:#000000; font-size: 14px; background-image:url('http://imgur.com/H79zWMS.png'&#41;; background-size: height cover; background-position: center center; background-size: height 200px; background-repeat:repeat-x scroll; text-decoration:none;)`,
  },
  {
    id: "blur-5",
    categoryId: "secret",
    order: 9,
    name: "블러 처리 5",
    memo: "이미지 패턴 가리기 (100px)",
    previewText: "블러 내용",
    keywords: ["블러", "스포일러", "비밀", "가리기"],
    code: `[?{블러 내용|}](#" style="color:#000000; font-size: 14px; background-image:url('http://imgur.com/H79zWMS.png'&#41;; background-size: height cover; background-position: center center; background-size: height 100px; background-repeat:repeat-x scroll; text-decoration:none;)`,
  },

  // ────────────────────────────────────────
  // scene
  // ────────────────────────────────────────
  {
    id: "divider",
    categoryId: "scene",
    order: 1,
    name: "가름선",
    memo: "장면 전환 구분선 — 선·장식·색상 수정 가능",
    previewText: "──── ✷ ────",
    keywords: ["가름선", "구분선", "장면", "전환"],
    code: `/desc [?{구분선|───────}](#" style="text-decoration:none; font-style: normal; color:#선색상;) [?{장식|✷}](#" style="font-style: normal; text-decoration:none; color: #장식색상;) [?{구분선|───────}](#" style="text-decoration:none; font-style: normal; color:#선색상;)`,
    fieldDefaults: {
      "color_선색상": "#333333",
      "color_장식색상": "#FF0053",
    },
  },
  {
    id: "chapter",
    categoryId: "scene",
    order: 2,
    name: "챕터 표시",
    memo: "챕터 번호와 챕터명 표시",
    previewText: "CHAPTER 1",
    keywords: ["챕터", "chapter", "장면", "제목"],
    code: `/desc [─────── CHAPTER ?{챕터 숫자|1} ───────](#" style="text-decoration:none; color: #챕터글자색; background-color:#배경색; text-align:center; display:block; padding:2px; box-shadow: 0px 8px 0px 15px #배경색; font-size:12px; font-style: normal;)[?{챕터명|챕터 제목}](#" style="text-decoration:none; color: white; background-color:#배경색; text-align:center; display:block; padding:2px; box-shadow: 0px 8px 0px 15px #배경색; font-style: normal;)`,
    fieldDefaults: {
      "color_챕터글자색": "#ffffff",
      "color_배경색": "#222222",
    },
  },
  {
    id: "bgm",
    categoryId: "scene",
    order: 3,
    name: "BGM",
    memo: "BGM 링크 버튼",
    previewText: "💿 BGM ▶ ■",
    keywords: ["bgm", "음악", "브금", "링크"],
    code: `/desc [💿　　?{BGM제목|BGM 제목}　　▶ ■](?{URL|https://} #" style="margin: 0px 7px 7px 7px; font-size:15px; color: black; background: white; text-align: center; display: inline-block; padding: 5px 15px; font-style: normal !important; font-weight: normal !important; border-radius: 20px; margin-left: 4px; text-decoration: none; border: 1px dashed black; box-shadow: 2px 2px 0 black; letter-spacing: -1px;)`,
  },
  {
    id: "bgm-v2",
    categoryId: "scene",
    order: 4,
    name: "BGM ver.2",
    memo: "색상 커스텀 BGM 버튼",
    previewText: "♬ ▶ ■",
    keywords: ["bgm", "음악", "브금", "링크", "ver2"],
    code: `/desc [♬　　▶ ■　────⊙───────　　0:00](?{URL|https://} #" style="font-size: 11px; font-family: gulim !important; color: #글자색; background: #배경색; text-align: center; display: inline-block; padding: 5px 15px; font-style: normal !important; font-weight: normal !important; border-radius: 20px; margin-left: -4px; text-decoration: none; border: 1px #선색; box-shadow: 2px 2px 0 #그림자색; letter-spacing: -1px;)`,
    fieldDefaults: {
      "color_글자색": "#333333",
      "color_배경색": "#ffffff",
      "color_선색": "#cccccc",
      "color_그림자색": "#999999",
    },
  },
  {
    id: "clock",
    categoryId: "scene",
    order: 5,
    name: "시계",
    memo: "시간·날짜 표시 위젯",
    previewText: "🕐 시간 표시",
    keywords: ["시계", "시간", "날짜"],
    code: `/desc [  ](#" style="content: ''; position: absolute; border: 10px solid transparent; border-color:#버튼색; border-bottom: 0;  margin: -1.5px 0px 0px  21px; text-decoration:none ; border-radius: 5px 5px 5px 5px; box-shadow: 0px 0px 0px black ) [?{시간|00:00 a.m.}　](#" style=" user-select:none; display: inline-block; line-height: 20px; text-decoration:none; position: relative; border:5px solid #바깥색; background:#안쪽색 ; color:#시간색 ; font-size: 14pt; border-radius: 10px 10px 10px 10px;  padding: 8px 54px 6px 8px; box-shadow: 0px 0px 0px black; font-style: normal !important;  )[　　　?{날짜|0000. 00. 00.}](#" style="position:absolute; width:100%; left:0px; font-weight:bold; text-decoration:none; font-size:5pt; padding:16.5px 0px 0px 28.5px; color: #날짜색; font-style: normal !important; )`,
    fieldDefaults: {
      "color_버튼색": "#444444",
      "color_바깥색": "#333333",
      "color_안쪽색": "#1a1a2e",
      "color_시간색": "#ffffff",
      "color_날짜색": "#aaaaaa",
    },
  },

  // ────────────────────────────────────────
  // info
  // ────────────────────────────────────────
  {
    id: "handout",
    categoryId: "info",
    order: 1,
    name: "핸드아웃",
    memo: "단서나 자료 표시",
    previewText: "🔎 핸드아웃",
    keywords: ["핸드아웃", "정보", "단서", "자료"],
    code: `/desc [**🔎 ?{핸드아웃 명|핸드아웃} │　** ?{내용|내용을 입력하세요}](#" style="text-decoration:none; color: #2c3e50; font-weight: bold; padding: 8px 12px; margin: 5px 5px 7px 5px; background: #ffffff; line-height: 150%; border: 2px solid darkgrey; box-shadow: -4px 4px 0px darkgrey; display: inline-block;)`,
  },
  {
    id: "investigation-point",
    categoryId: "info",
    order: 2,
    name: "조사 포인트",
    memo: "조사 가능한 지점 표시",
    previewText: "◈ 조사 내용",
    keywords: ["조사", "포인트", "단서", "정보"],
    code: `/desc [?{장식|◈}](#" style="color:#장식색상; font-style: normal; text-decoration:none;) [?{조사 내용|조사할 수 있는 내용을 입력하세요}](#" style="color:#333333;font-style: normal; text-decoration:none;)`,
    fieldDefaults: {
      "color_장식색상": "#FF0053",
    },
  },
  {
    id: "thin-text-two-lines",
    categoryId: "info",
    order: 3,
    name: "얇은 글씨 (두 줄)",
    memo: "캐치프레이즈 + 설명 두 줄",
    previewText: "캐치프레이즈",
    keywords: ["얇은 글씨", "캐치프레이즈", "두줄", "정보"],
    code: `/desc  [?{첫 번째 줄|시나리오의 캐치프레이즈를}](#" style="text-decoration: none; color:#333333; font-size:12px; font-weight: normal;) [?{두 번째 줄|여기에 적으면 괜찮을 것 같아요}](#" style="color:#333333;display:block;text-decoration:none; font-size:12px; font-weight: normal;)`,
  },
  {
    id: "thin-text-one-line",
    categoryId: "info",
    order: 4,
    name: "얇은 글씨 (한 줄)",
    memo: "한 줄 얇은 글씨",
    previewText: "얇은 글씨",
    keywords: ["얇은 글씨", "한줄", "정보"],
    code: `/desc [?{내용|물론 한줄로도 됩니다}](#" style="text-decoration: none; color:#333333; font-size:12px; font-weight: normal;)`,
  },
  {
    id: "kpc-pc",
    categoryId: "info",
    order: 5,
    name: "KPC / PC 표시",
    memo: "KPC·PC 이름 및 생존 여부 표시",
    previewText: "KPC / PC",
    keywords: ["kpc", "pc", "이름", "생존"],
    code: `/desc [KPC](#" style="font-style: normal; text-decoration:none; color: #강조색;) [?{KPC 이름|KPC 이름}](#" style="font-style: normal; text-decoration:none; color: #333333;)　[PC](#" style="font-style: normal; text-decoration:none; color: #강조색;) [?{PC 이름|PC 이름}](#" style="font-style: normal; text-decoration:none; color: #333333;)`,
    fieldDefaults: {
      "color_강조색": "#FF0053",
    },
  },
  {
    id: "writer",
    categoryId: "info",
    order: 6,
    name: "라이터",
    memo: "시나리오 작성자 표시",
    previewText: "Written by",
    keywords: ["라이터", "작가", "시나리오", "written"],
    code: `/desc [Written by](#" style="font-style: normal; text-decoration:none; color: #강조색;) [?{라이터 이름|이름}](#" style="font-style: normal; text-decoration:none; color: #333333;)`,
    fieldDefaults: {
      "color_강조색": "#FF0053",
    },
  },

  // ────────────────────────────────────────
  // decorate
  // ────────────────────────────────────────
  {
    id: "highlight",
    categoryId: "decorate",
    order: 1,
    name: "강조 텍스트",
    memo: "색상·기울임·굵게 조합 — 버튼으로 스타일 선택",
    previewText: "할말",
    keywords: ["강조", "색상", "기울임", "굵게", "볼드", "꾸밈"],
    code: `[?{할말|할말}](#" style="text-decoration:none; color: #색상코드;)`,
    variants: [
      {
        id: "basic",
        name: "기본",
        code: `[?{할말|할말}](#" style="text-decoration:none; color: #색상코드;)`,
      },
      {
        id: "italic",
        name: "기울임",
        code: `[?{할말|할말}](#" style="text-decoration:none; font-style:italic; color: #색상코드;)`,
      },
      {
        id: "bold",
        name: "굵게",
        code: `[?{할말|할말}](#" style="text-decoration:none; font-weight:bold; color: #색상코드;)`,
      },
      {
        id: "bold-italic",
        name: "굵게+기울임",
        code: `[?{할말|할말}](#" style="text-decoration:none; font-style:italic; font-weight:bold; color: #색상코드;)`,
      },
      {
        id: "underline",
        name: "밑줄",
        code: `[?{할말|할말}](#" style="text-decoration:underline; color: #색상코드;)`,
      },
      {
        id: "overline",
        name: "윗줄",
        code: `[?{할말|할말}](#" style="text-decoration:overline; color: #색상코드;)`,
      },
    ],
    fieldDefaults: {
      "color_색상코드": "#FF0053",
    },
  },
  {
    id: "dashed-box",
    categoryId: "decorate",
    order: 2,
    name: "점선 네모 강조",
    memo: "점선 테두리 박스",
    previewText: "점선 박스",
    keywords: ["점선", "네모", "박스", "강조"],
    code: `[?{내용|내용}](#" style="text-decoration:none; color:black; border:1px dashed black; padding:2px;)`,
  },
  {
    id: "background-color",
    categoryId: "decorate",
    order: 3,
    name: "배경색 변경",
    memo: "배경색이 있는 텍스트 블록",
    previewText: "배경색 텍스트",
    keywords: ["배경색", "배경", "강조", "색상"],
    code: `/desc [?{내용|내용}](#" style="text-decoration:none; color: #글자색; background-color:#배경색; text-align:center; display:block; padding:2px; box-shadow: 0px 8px 0px 15px #배경색;)`,
    fieldDefaults: {
      "color_글자색": "#ffffff",
      "color_배경색": "#B9062F",
    },
  },
  {
    id: "background-highlight",
    categoryId: "decorate",
    order: 4,
    name: "배경 강조",
    memo: "배경색이 있는 인라인 강조",
    previewText: "배경 강조",
    keywords: ["배경", "강조", "인라인"],
    code: `[?{할말|할말}](#" style="text-decoration:none; color: #글자색; background-color:#배경색; text-align:center; display:block; padding:4px; line-height:1.3;)`,
    fieldDefaults: {
      "color_글자색": "#ffffff",
      "color_배경색": "#B9062F",
    },
  },
  {
    id: "neon-sign",
    categoryId: "decorate",
    order: 5,
    name: "네온 사인",
    memo: "네온 효과 텍스트",
    previewText: "네온 사인",
    keywords: ["네온", "사인", "glow", "효과"],
    code: `[?{내용|네온 사인}](#" style="text-decoration:none; display:block; text-align: center; font-family: Georgia, serif; font-weight: bold; font-style: italic; font-size:15px; line-height:1.3; margin: 0px 0px 7px 0; color: white; text-shadow: 0 0 2px white, 0 0 3px #네온색, 0 0 10px #네온색, 0 0 15px #네온색;" )`,
    fieldDefaults: {
      "color_네온색": "#FF0053",
    },
  },

  // ────────────────────────────────────────
  // message
  // ────────────────────────────────────────
  {
    id: "speech-bubble",
    categoryId: "message",
    order: 1,
    name: "기본 말풍선",
    memo: "꼬리가 있는 기본 말풍선",
    previewText: "할말",
    keywords: ["말풍선", "대화", "채팅"],
    code: `[?{할말|할말}](#" style="position: relative; background: #말풍선색; border-radius: .4em; padding:6px; user-select:none; display:block; line-height:1.5; text-decoration:none;) [ ](#" style="content: ''; position: absolute; border: 12px solid transparent; border-right-color: #말풍선색; border-left: 0; border-bottom: 0; margin-top: -25px; margin-left: -10px; line-height:1.3; text-decoration:none;)`,
    fieldDefaults: {
      "color_말풍선색": "#ffffff",
    },
  },
  {
    id: "kakao-received",
    categoryId: "message",
    order: 2,
    name: "카톡 받은 메시지",
    memo: "카카오톡 스타일 — 흰색 말풍선",
    previewText: "받은 메시지",
    keywords: ["카톡", "받은", "메시지", "하얀색"],
    code: `[?{내용|받은 메시지}](#" style="color:black; background: white; border-radius: 0px 15px 15px 15px; padding: 8px; display: inline-block; line-height:1.3; box-shadow: 1px 1px 1px 1px #97A9B9; text-align:right; text-decoration:none; margin: 0px 0px 3px 0px; word-break:keep-all;)　 [?{오전오후|PM|AM}](#" style="text-decoration:none; text-align:right; color:black; font-size:10px;) [?{시간|12:00}](#" style="text-decoration:none; text-align:right; color: black; font-size:10px;)`,
  },
  {
    id: "kakao-sent",
    categoryId: "message",
    order: 3,
    name: "카톡 보낸 메시지",
    memo: "카카오톡 스타일 — 노란색 말풍선",
    previewText: "보낸 메시지",
    keywords: ["카톡", "보낸", "메시지", "노란색"],
    code: `[?{오전오후|PM|AM}](#" style="text-decoration:none; text-align:left; color:black; font-size:9px;) [ ?{시간|12:00}](#" style="text-decoration:none; line-height:1.3; text-align:left; color: black; font-size:10px;)　[?{내용|보낸 메시지}](#" style="color:black; background: #FFEB33; border-radius: 15px 0px 15px 15px; padding: 8px; display: inline-block; line-height:1.3; box-shadow: 1px 1px 1px 1px #97A9B9; text-align:left; text-decoration:none; margin: 0px 0px 3px 0px; word-break:keep-all;)`,
  },
  {
    id: "iphone-sent-blue",
    categoryId: "message",
    order: 4,
    name: "아이폰 보낸 메시지 (파랑)",
    memo: "iMessage 스타일 — 파란색 말풍선",
    previewText: "보낸 메시지",
    keywords: ["아이폰", "imessage", "파란색", "보낸"],
    code: `[?{내용|보낸 메시지}](#" style="color:white; background: #3478F6; border-radius: 15px 15px 15px 0px; padding: 8px; display: inline-block; line-height:1.3; box-shadow: 1px 1px 1px 1px #97A9B9; text-align:left; text-decoration:none; margin: 0px 0px 3px 0px; word-break:keep-all;)　[?{오전오후|PM|AM}](#" style="text-decoration:none; line-height:1.3; text-align:left; color:black; font-size:9px;) [ ?{시간|12:00}](#" style="text-decoration:none; line-height:1.3; text-align:left; color: black; font-size:9px;)`,
  },
  {
    id: "iphone-received-white",
    categoryId: "message",
    order: 5,
    name: "아이폰 받은 메시지 (흰색)",
    memo: "iMessage 스타일 — 흰색 말풍선",
    previewText: "받은 메시지",
    keywords: ["아이폰", "imessage", "흰색", "받은"],
    code: `[?{오전오후|PM|AM}](#" style="text-decoration:none; line-height:1.3; text-align:right; color:black; font-size:9px;) [ ?{시간|12:00}](#" style="text-decoration:none; line-height:1.3; text-align:right; color: black; font-size:9px;)　[?{내용|받은 메시지}](#" style="color:black; background: #F8F8F8; border-radius: 15px 15px 0px 15px; padding: 8px; display: inline-block; line-height:1.3; box-shadow: 1px 1px 1px 1px #97A9B9; text-align:right; text-decoration:none; margin: 0px 0px 3px 0px; word-break:keep-all;)`,
  },
  {
    id: "iphone-sent-green",
    categoryId: "message",
    order: 6,
    name: "아이폰 보낸 메시지 (초록)",
    memo: "SMS 스타일 — 초록색 말풍선",
    previewText: "보낸 메시지",
    keywords: ["아이폰", "sms", "초록색", "보낸"],
    code: `[?{내용|보낸 메시지}](#" style="color:black; background: #F8F8F8; border-radius: 15px 15px 15px 0px; padding: 8px; display: inline-block; line-height:1.3; box-shadow: 1px 1px 1px 1px #97A9B9; text-align:left; text-decoration:none; margin: 0px 0px 3px 0px; word-break:keep-all;)　[?{오전오후|PM|AM}](#" style="text-decoration:none; line-height:1.3; text-align:left; color:black; font-size:9px;) [ ?{시간|12:00}](#" style="text-decoration:none; line-height:1.3; text-align:left; color: black; font-size:9px;)`,
  },
  {
    id: "line-received",
    categoryId: "message",
    order: 7,
    name: "라인 받은 메시지",
    memo: "LINE 스타일 — 초록색 말풍선",
    previewText: "받은 메시지",
    keywords: ["라인", "line", "초록", "받은"],
    code: `[?{내용|받은 메시지}](#" style="text-decoration:none; color: black; background: #C0EC95; border: 5px solid: #C0EC95; border-radius: 20px 0px 20px 20px; padding: 8px; margin: 0px 0px 3px 0px; box-shadow: 1px 1px 1px 1px #799096; display: inline-block; line-height:1.3; text-align:right; word-break:keep-all;) [?{시간|12:00}](#" style="text-decoration:none; color: #666666; font-size:9px;) [읽음](#" style="text-decoration:none; color: #666666; font-size:9px;)`,
  },
  {
    id: "line-sent",
    categoryId: "message",
    order: 8,
    name: "라인 보낸 메시지",
    memo: "LINE 스타일 — 흰색 말풍선",
    previewText: "보낸 메시지",
    keywords: ["라인", "line", "흰색", "보낸"],
    code: `[읽음](#" style="text-decoration:none; color: #666666; font-size:9px;) [?{시간|12:00}](#" style="text-decoration:none; color: #666666; font-size:9px;)　[?{내용|보낸 메시지}](#" style="text-decoration:none; color: black; background: #F1F1F1; border: 5px solid: #F1F1F1; border-radius: 0px 20px 20px 20px; box-shadow: 1px 1px 1px 1px #799096; padding: 8px; display: inline-block; margin: 0px 0px 3px 0px; line-height:1.3; text-align:left; word-break:keep-all;)`,
  },

  // ────────────────────────────────────────
  // effect
  // ────────────────────────────────────────
  {
    id: "madness-current",
    categoryId: "effect",
    order: 1,
    name: "광기 현재화",
    memo: "피시명과 광기 현재화 연출 — 네온 색상 수정 가능",
    previewText: "광기 현재화",
    keywords: ["광기", "현재화", "인세인", "연출"],
    code: `/desc [?{피시명|캐릭터명}](#" style="text-decoration:none; font-size: 17px; font-weight: bold; text-shadow:1px 2px 1px #네온색1,-2px 2px 1px #네온색2)의 [광기 현재화](#" style="text-decoration:none; font-size: 17px; text-shadow:1px 1px 1px #네온색1,-2px 1px 1px #네온색2)`,
    fieldDefaults: {
      "color_네온색1": "#77FFC8",
      "color_네온색2": "#DB91FF",
    },
  },
  {
    id: "gauge-hp",
    categoryId: "effect",
    order: 2,
    name: "게이지 / 체력바",
    memo: "퍼센트 선택형 체력바",
    previewText: "체력바",
    keywords: ["게이지", "체력바", "hp", "퍼센트"],
    code: `/desc ?{퍼센트| 100%,[100%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-color: #C9151E&#41;| 90%,[90%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 90%&#44; #000000 90%&#41;| 80%,[80%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 80%&#44; #000000 80%&#41;| 70%,[70%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 70%&#44; #000000 70%&#41;| 60%,[60%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 60%&#44; #000000 60%&#41;| 50%,[50%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 50%&#44; #000000 50%&#41;| 40%,[40%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 40%&#44; #000000 40%&#41;| 30%,[30%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 30%&#44; #000000 30%&#41;| 20%,[20%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 20%&#44; #000000 20%&#41;| 10%,[10%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-image: linear-gradient(to right&#44; #C9151E 10%&#44; #000000 10%&#41;| 0%,[0%](#" style="color:#ffffff; border:1px solid black; margin:1px;text-align:center;user-select:none;display:block;padding:2px;text-decoration:none;background-color: #000000&#41;}`,
  },

  // ────────────────────────────────────────
  // custom
  // ────────────────────────────────────────
  {
    id: "custom-empty",
    categoryId: "custom",
    order: 1,
    name: "사용자 커스텀",
    memo: "직접 만든 매크로 저장용",
    previewText: "직접 만든 매크로",
    keywords: ["커스텀", "사용자", "저장", "직접"],
    code: "",
  },
];
